	...
	//	model.sys.nav_state  = Status.NAVIGATION_STATE_MANUAL;
		...
		if(model.sys.isStatus(Status.MSP_ARMED)) {
           ...
           // synchronized( this) {
			...
			else if(MAV_CUST_MODE.is(hb.custom_mode,MAV_CUST_MODE.PX4_CUSTOM_MAIN_MODE_AUTO, MAV_CUST_MODE.PX4_CUSTOM_SUB_MODE_AUTO_LAND))
				model.sys.nav_state = Status.NAVIGATION_STATE_AUTO_LAND;

			else if(MAV_CUST_MODE.is(hb.custom_mode,MAV_CUST_MODE.PX4_CUSTOM_MAIN_MODE_AUTO, MAV_CUST_MODE.PX4_CUSTOM_SUB_MODE_AUTO_TAKEOFF))
				model.sys.nav_state = Status.NAVIGATION_STATE_AUTO_TAKEOFF;

			else if(MAV_CUST_MODE.is(hb.custom_mode,MAV_CUST_MODE.PX4_CUSTOM_MAIN_MODE_AUTO, MAV_CUST_MODE.PX4_CUSTOM_SUB_MODE_AUTO_LOITER))
				model.sys.nav_state = Status.NAVIGATION_STATE_AUTO_LOITER;

			else if(MAV_CUST_MODE.is(hb.custom_mode,MAV_CUST_MODE.PX4_CUSTOM_MAIN_MODE_AUTO, MAV_CUST_MODE.PX4_CUSTOM_SUB_MODE_AUTO_MISSION))
